...
def _templates(name: str, *, witness: bool = True):
    ...
    if witness:
        docs["witness/README.md"] = """# Witness write-back\n\nWitness is an optional append-only record for carrying a compact account of a work session into a later handoff. It checks **form and declared provenance boundaries, not truth or evidential sufficiency**.\n\nThe normal operator flow is:\n\n```bash\noxbow witness draft --stream witness/stream.json --out witness/prompt.md\n# give prompt.md to the model that just did the work; save its JSON as packet.json\noxbow witness validate packet.json\noxbow witness append packet.json --stream witness/stream.json\n```\n\nPacket v2 is the default. It gives each `read` an explicit status, scope, and basis; makes unresolved `overhang` structured; adds a packet-level claim boundary; and optionally carries source anchors, weather, audit exceptions, and lineage. `quick`, `standard`, and `deep` drafting depths all emit the same v2 packet contract.\n\n```bash\noxbow witness draft --depth quick --stream witness/stream.json\noxbow witness draft --depth standard --stream witness/stream.json\noxbow witness draft --depth deep --stream witness/stream.json\n```\n\nLegacy packet v1 remains valid and can still be drafted with `--packet-version v1`. Packet v1 and v2 may coexist in the same `oxbow-witness-stream-v1` stream.\n\nA `basis` is a declared referent, not proof that the referent supports the read. A `promoted` read records workstream standing, not a truth verdict. Records are append-only. Corrections are new records; old records are not rewritten. The stream's `derived_index` is rebuildable and never outranks the records.\n"""
        stream = {
